...
v2v1 = get_cohort(mdata, "v2v1")
v3v1 = get_cohort(mdata, "v3v1")
v3v2 = get_cohort(mdata, "v3v2")

v2v1_func = let comm = unirefs[:, v2v1.seqprep]
  mat = collect(abundances(comm)')
  ixs = ThreadsX.map(col -> sum(col) > 0, eachcol(mat))
  hcat(v2v1, DataFrame(mat[:, ixs], featurenames(comm)[ixs]))
end

v3v1_func = let comm = unirefs[:, v3v1.seqprep]
  mat = collect(abundances(comm)')
  ixs = ThreadsX.map(col -> sum(col) > 0, eachcol(mat))
  hcat(v3v1, DataFrame(mat[:, ixs], featurenames(comm)[ixs]))
end

v3v2_func = let comm = unirefs[:, v3v2.seqprep]
  mat = collect(abundances(comm)')
  ixs = ThreadsX.map(col -> sum(col) > 0, eachcol(mat))
  hcat(v3v2, DataFrame(mat[:, ixs], featurenames(comm)[ixs]))
end


for feature in eeg_features
  @info feature
  EEGMicrobiome.runlms(v2v1_func, "./data/outputs/lms/$(feature)_v2v1_lms.csv", feature, names(v2v1_func, r"^UniRef");
    additional_cols=[:age_diff], age_col="stool_age",
    formula=term(:func) ~ term(feature) + term(:stool_age) + term(:age_diff) + term(:n_trials)
  )
end
...
for feature in eeg_features
  @info feature
  EEGMicrobiome.runlms(v3v1_func, "./data/outputs/lms/$(feature)_v3v1_lms.csv", feature, names(v3v1_func, r"^UniRef");
    additional_cols=[:age_diff], age_col="stool_age",
    formula=term(:func) ~ term(feature) + term(:stool_age) + term(:age_diff) + term(:n_trials)
  )
end
...
for feature in eeg_features
  @info feature
  EEGMicrobiome.runlms(v3v2_func, "./data/outputs/lms/$(feature)_v3v2_lms.csv", feature, names(v3v2_func, r"^UniRef");
    additional_cols=[:age_diff], age_col="stool_age",
    formula=term(:func) ~ term(feature) + term(:stool_age) + term(:age_diff) + term(:n_trials)
  )
end

rftps = ("v2v1", "v3v1", "v3v2")

#-

fsea_df = let fsea_df = DataFrame()
  for tp in rftps, feature in eeg_features
    filepath = "./data/outputs/lms/$(feature)_$(tp)_lms.csv"
    lms = CSV.read(filepath, DataFrame)
