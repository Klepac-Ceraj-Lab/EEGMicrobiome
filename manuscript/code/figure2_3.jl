...
#-
basepath = "/home/guilherme/Repos/Leap"

age_predictions = CSV.read(joinpath(basepath, "results", "2024_01_14_abundance_allvars", "leap_age_residuals.csv"), DataFrame)

brainrise_ecbq_table = @chain CSV.read(joinpath(basepath, "ext_data", "ECBQ", "BRAINRISE_ECBQ_data.csv"), DataFrame) begin
    transform!( :idade_crianca_dias => (x -> x ./ 365) => :age_assessment_years; renamecols = false )
    transform!( :individualID => (x -> "brainrise-" .* [ split(el, '_')[1] for el in x ]) => :individualID; renamecols = false )
    select!( [ "individualID", "age_assessment_years", "SHEFFCO", "shortinh", "shortats", "shortatf", "shortsth", "shortfru" ] )
    rename!(
        "individualID" => "subject_id",
        "SHEFFCO" => "ecbq_effortful_control",
        "shortinh" => "ecbq_inhibitory_control",
        "shortats" => "ecbq_attentional_shift",
        "shortatf" => "ecbq_attentional_focusing",
        "shortsth" => "ecbq_soothability",
        "shortfru" => "ecbq_frustration"
    )
    dropmissing()
end

khula_ecbq_table = @chain CSV.read(joinpath(basepath, "ext_data", "ECBQ", "KhulaSA_ECBQ_data.csv"), DataFrame) begin
    transform!( :ecbq_t4_age => (x -> x ./ 365) => :age_assessment_years; renamecols = false )
    transform!( :subject_id => (x -> "khula-" .* x) => :subject_id; renamecols = false )
    select!( [ "subject_id", "age_assessment_years", "SHEFFCO", "shortinh", "shortats", "shortatf", "shortsth", "shortfru" ] )
    rename!(
        "SHEFFCO" => "ecbq_effortful_control",
        "shortinh" => "ecbq_inhibitory_control",
        "shortats" => "ecbq_attentional_shift",
...
ecbqs_table = vcat(brainrise_ecbq_table,khula_ecbq_table )

joined_df = leftjoin(ecbqs_table, unique(DataFrames.combine(groupby(age_predictions, ["subject_id", "visit"]),
                                                "test_prediction"=> mean=>"predicted_age"), "subject_id"), on="subject_id")

transform!(joined_df, AsTable(["age_assessment_years", "predicted_age"])=> ByRow(nt-> nt[1] - nt[2]) =>"pad")
dropmissing!(joined_df, "pad")                
transform!(joined_df, "subject_id"=> ByRow(s-> first(split(s, "-")))=> "cohort")
transform!(groupby(joined_df, "cohort"), "pad" => (pad-> begin
    qs = quantile(pad, 0.1 .* collect(1:9))
    map(pad) do p
        idx = findfirst(>=(p), qs)
        isnothing(idx) ? 10 : idx
    end
end) => "pred_quantile")

for subscale in names(joined_df, r"^ecbq_")
    transform!(groupby(joined_df, "cohort"), subscale => (ss-> begin
    qs = quantile(ss, 0.25 .* collect(1:3))
        map(ss) do p
            idx = findfirst(>=(p), qs)
            isnothing(idx) ? 4 : idx
        end
    end) => "$(subscale)_quantile")
end



select(joined_df, ["age_assessment_years", "predicted_age", "pad", "pred_quantile"])


for subscale in setdiff(names(joined_df, r"^ecbq_"), names(joined_df, r"^ecbq_.+quantile"))
    fig = Figure()
    ax = Axis(fig[1,1]; title = subscale, xlabel="PAD quantile")
    violin!(ax, joined_df.pred_quantile, joined_df[!, subscale]; color=(:gray,0.2))
    scatter!(ax, joined_df.pred_quantile .+ rand(Normal(0., 0.05), size(joined_df, 1)), joined_df[!, subscale];
             color=[q == 4 ? :red : q == 1 ? :blue : :gray for q in joined_df[!, "$(subscale)_quantile"]])
    save("/home/kevin/Downloads/pad_ecbq_$subscale.png", fig)
end
