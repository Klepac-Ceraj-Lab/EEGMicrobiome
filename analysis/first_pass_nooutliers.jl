...
# Remove outliers by eye from RF output plots
subset!(eeg, "peak_amp_N1"=> ByRow(>(-10)))
subset!(eeg, "peak_latency_P1"=> ByRow(<(125)))
subset!(eeg, "peak_amp_P1"=> ByRow(a-> -5 < a < 13))
...
for (tp, df) in zip(tps, mbotps)
    ...
    for feature in eeg_features
        EEGMicrobiome.runlms(subeeg, "./data/outputs/lms/$(feature)_$(tp)_lms_nooutliters.csv", feature, names(subeeg, r"^UniRef"))
    end
end
...
let
    ...
    subset!(subeeg, "sample"=> ByRow(!ismissing))
    for feature in eeg_features
        EEGMicrobiome.runlms(subeeg, "./data/outputs/lms/$(feature)_$(tp)_future6m_lms_nooutliters.csv", feature, names(subeeg, r"^UniRef");
...
let
    ...
    transform!(subeeg, AsTable(["age", "eeg_age"])=> ByRow(nt-> nt.eeg_age - nt.age) => "age_diff")
    for feature in eeg_features
        EEGMicrobiome.runlms(subeeg, "./data/outputs/lms/$(feature)_$(tp)_future12m_lms_nooutliters.csv", feature, names(subeeg, r"^UniRef");
            additional_cols = [:age_diff],
            formula = term(:func) ~ term(feature) + term(:age) + term(:age_diff) + term(:n_segments)
        )
    end
...
fsea_df = let fsea_df = DataFrame()
    for tp in tps, feature in eeg_features
        filepath = "./data/outputs/lms/$(feature)_$(tp)_lms_nooutliters.csv"
...
sort!(fsea_df, "q₀")
CSV.write("data/outputs/fsea/true_ages_fsea_nooutliters.csv", fsea_df)
# fsea_df = CSV.read("data/outputs/fsea/true_ages_fsea_nooutliters.csv", DataFrame)
...
future12m_fsea_df = let fsea_df = DataFrame()
    for tp in ("3m", "6m"), feature in eeg_features
        ...
        filepath = "./data/outputs/lms/$(feature)_$(tp)_future12m_lms_nooutliters.csv"
        lms = CSV.read(filepath, DataFrame)
...
CSV.write("data/outputs/fsea/future12m_true_ages_fsea_nooutliters.csv", future12m_fsea_df)
# future12m_fsea_df = CSV.read("data/outputs/fsea/future12m_true_ages_fsea_nooutliters.csv", DataFrame)
...
future6m_fsea_df = let fsea_df = DataFrame()
    for feature in eeg_features
        # contains(feature, "corrected") && continue
        tp = "3m"
        filepath = "./data/outputs/lms/$(feature)_$(tp)_future6m_lms_nooutliters.csv"
        lms = CSV.read(filepath, DataFrame)
...
CSV.write("data/outputs/fsea/future6m_true_ages_fsea_nooutliters.csv", future6m_fsea_df)
# future6m_fsea_df = CSV.read("data/outputs/fsea/future6m_true_ages_fsea_nooutliters.csv", DataFrame)
